myApp.controller("JobController", function($scope,$http,$route,$location,$rootScope) {
	...
	$scope.addJob=function() {
	      ...
	      $http.post('http://localhost:8090/Middleware/addJob',$scope.job)
	...
	$scope.deleteJob=function(jobId) {
		 $http.get('http://localhost:8090/Middleware/deleteJob/'+jobId)
		 .then(function(response) {
	...
	function listJobs() 
	{
		...
		$http.get('http://localhost:8090/Middleware/getJobs')
		.then(function(response) {

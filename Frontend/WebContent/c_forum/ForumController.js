myApp.controller("ForumController", function($scope,$location,$rootScope,$http,$route) {
	...
	$scope.addForum=function() {
	      ...
	      $http.post('http://localhost:8090/Middleware/addForum',$scope.forum)
	...
	{
		$http.post('http://localhost:8090/Middleware/updateForum',$scope.forum)
		.then(function(response) {
	...
	$scope.deleteForum=function(forumId) {
		 $http.get('http://localhost:8090/Middleware/deleteForum/'+forumId)
		 .then(function(response) {
	...
	$scope.approveForum=function(forumId) {
		 $http.get('http://localhost:8090/Middleware/approveForum/'+forumId)
		 .then(function(response) {
	...
	$scope.rejectForum=function(forumId) {
		 $http.get('http://localhost:8090/Middleware/rejectForum/'+forumId)
		 .then(function(response) {
	...
	function listForums() 
	{
		...
		$http.get('http://localhost:8090/Middleware/getForums')
		.then(function(response) {
	...
	function getForum()
	{
		$http.get('http://localhost:8090/Middleware/getForum/'+$rootScope.forumId)
		.then(function(response) {

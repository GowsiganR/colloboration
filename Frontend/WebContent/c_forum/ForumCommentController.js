myApp.controller("ForumCommentController", function($scope,$location,$rootScope,$http,$route) {
	...
	{
		$http.get('http://localhost:8090/Middleware/getForumComments/'+$rootScope.forumComId)
	...
	$scope.deleteForumComment=function(commentId) {
		 $http.get('http://localhost:8090/Middleware/deleteForumComment/'+commentId)
		 .then(function(response) {
	...
	$scope.updateForumComment=function() {
		 $http.post('http://localhost:8090/Middleware/updateForumComment',$scope.forumcomment)
		 .then(function(response) {
	...
	$scope.addComment=function(forumId) {
	      ...
	      $http.post('http://localhost:8090/Middleware/addForumComment',$scope.forumComment)
	      .then(function(response) {
	...
	{
		$http.get('http://localhost:8090/Middleware/getForumComment/'+$rootScope.commentId)
		.then(function(response) {

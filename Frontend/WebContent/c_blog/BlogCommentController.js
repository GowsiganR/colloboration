myApp.controller("BlogCommentController", function($scope,$location,$rootScope,$http,$route) {
	...
	{
		$http.get('http://localhost:8090/Middleware/getBlogComments/'+$rootScope.blogComId)
	...
	$scope.deleteBlogComment=function(commentId) {
		 $http.get('http://localhost:8090/Middleware/deleteBlogComment/'+commentId)
		 .then(function(response) {
	...
	$scope.updateBlogComment=function() {
		 $http.post('http://localhost:8090/Middleware/updateBlogComment',$scope.blogcomment)
		 .then(function(response) {
	...
	$scope.addComment=function(blogId) {
	      ...
	      $http.post('http://localhost:8090/Middleware/addBlogComment',$scope.blogComment)
	      .then(function(response) {
	...
	{
		$http.get('http://localhost:8090/Middleware/getBlogComment/'+$rootScope.commentId)
		.then(function(response) {
